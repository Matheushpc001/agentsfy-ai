...
async function handleCreateInstance(supabase, params) {
  ...
  try {
    ...
    const { data: globalConfigs, error: globalError } = await supabase.from('evolution_global_configs').select('*').eq('is_active', true).order('created_at', {
      ascending: false
    }).limit(1);
    if (globalError) {
    ...
    const { data: config, error: configError } = await supabase.from('evolution_api_configs').insert([
      configData
    ]).select().single();
    if (configError) {
    ...
    try {
      ...
      await supabase.from('evolution_api_configs').delete().eq('id', config.id);
...
async function configureWebhook(globalConfig, instanceName) {
  try {
...
async function handleConnectInstance(supabase, params) {
  ...
  try {
    ...
    if (qrCode) {
      ...
      const statusCheck = await handleCheckStatus(supabase, {
        config_id
      });
      return statusCheck;
    }
...
async function handleCheckStatus(supabase, params) {
  const { config_id } = params;
...
function mapEvolutionStatus(evolutionStatus) {
